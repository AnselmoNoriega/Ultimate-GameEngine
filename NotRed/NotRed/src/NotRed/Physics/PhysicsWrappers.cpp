...
namespace NR
{
	...
	void ContactListener3D::onWake(physx::PxActor** actors, physx::PxU32 count)
	{
		for (uint32_t i = 0; i < count; ++i)
		{
			...
			NR_CORE_INFO("PhysX Actor waking up: ID: {0}, Name: {1}", (uint64_t)entity.GetID(), entity.GetComponent<TagComponent>().Tag);
		}
	}
	...
	void ContactListener3D::onSleep(physx::PxActor** actors, physx::PxU32 count)
	{
		for (uint32_t i = 0; i < count; ++i)
		{
			...
			NR_CORE_INFO("PhysX Actor going to sleep: ID: {0}, Name: {1}", (uint64_t)entity.GetID(), entity.GetComponent<TagComponent>().Tag);
		}
	}
	...
	physx::PxScene* PhysicsWrappers::CreateScene()
	{
		if (!sCpuDispatcher)
		{
			sCpuDispatcher = physx::PxDefaultCpuDispatcherCreate(1);
		}
		...
		sceneDesc.cpuDispatcher = sCpuDispatcher;
	...
	void PhysicsWrappers::AddBoxCollider(PhysicsActor& actor)
	{
		...
		glm::vec3 colliderSize = actor.mEntity.Transform().Scale * collider.Size;
	...
	void PhysicsWrappers::AddSphereCollider(PhysicsActor& actor)
	{
		...
		glm::vec3 actorScale = actor.mEntity.Transform().Scale;
	...
	std::vector<physx::PxShape*> PhysicsWrappers::CreateConvexMesh(MeshColliderComponent& collider, const glm::vec3& scale, bool invalidateOld)
	{
		...
		if (!PhysicsMeshSerializer::IsSerialized(collider.CollisionMesh->GetFilePath()))
		{
			...
			for (const auto& submesh : collider.CollisionMesh->GetSubmeshes())
			{
				...
				physx::PxConvexMeshGeometry convexGeometry = physx::PxConvexMeshGeometry(convexMesh, physx::PxMeshScale(ToPhysicsVector(submeshScale * scale)));
	...
	std::vector<physx::PxShape*> PhysicsWrappers::CreateTriangleMesh(MeshColliderComponent& collider, const glm::vec3& scale, bool invalidateOld)
	{
		...
		if (collider.ProcessedMeshes.size() <= 0)
		{
			for (auto shape : shapes)
			{
				...
				glm::mat4 scale = glm::scale(glm::mat4(1.0f), FromPhysicsVector(triangleGeometry.scale.scale));
