...
namespace NR
{
	...
	void ContactListener::onContact(const physx::PxContactPairHeader& pairHeader, const physx::PxContactPair* pairs, physx::PxU32 nbPairs)
	{
		...
		{
			if (!removedActorA && ScriptEngine::IsEntityModuleValid(actorA->GetEntity())) ScriptEngine::CollisionBegin(actorA->GetEntity());
			if (!removedActorB && ScriptEngine::IsEntityModuleValid(actorB->GetEntity())) ScriptEngine::CollisionBegin(actorB->GetEntity());
		}
		else if (pairs->flags == physx::PxContactPairFlag::eACTOR_PAIR_LOST_TOUCH)
		{
			if (!removedActorA && ScriptEngine::IsEntityModuleValid(actorA->GetEntity())) ScriptEngine::CollisionEnd(actorA->GetEntity());
			if (!removedActorB && ScriptEngine::IsEntityModuleValid(actorB->GetEntity())) ScriptEngine::CollisionEnd(actorB->GetEntity());
		}
	}
	...
	void ContactListener::onTrigger(physx::PxTriggerPair* pairs, physx::PxU32 count)
	{
		...
		{
			if (triggerActor && ScriptEngine::IsEntityModuleValid(triggerActor->GetEntity()))	 ScriptEngine::TriggerBegin(triggerActor->GetEntity());
			if (otherActor && ScriptEngine::IsEntityModuleValid(otherActor->GetEntity()))		 ScriptEngine::TriggerBegin(otherActor->GetEntity());
		}
		else if (pairs->status == physx::PxPairFlag::eNOTIFY_TOUCH_LOST)
		{
			if (triggerActor && ScriptEngine::IsEntityModuleValid(triggerActor->GetEntity()))	 ScriptEngine::TriggerEnd(triggerActor->GetEntity());
			if (otherActor && ScriptEngine::IsEntityModuleValid(otherActor->GetEntity()))		 ScriptEngine::TriggerEnd(otherActor->GetEntity());
		}
	}
