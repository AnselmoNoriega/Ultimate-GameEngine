...
namespace NR
{
	NR::Ref<NR::Texture2D> SceneHierarchyPanel::sPencilIcon;
	NR::Ref<NR::Texture2D> SceneHierarchyPanel::sPlusIcon;
	NR::Ref<NR::Texture2D> SceneHierarchyPanel::sGearIcon;
		...
		{
			{
				...
				// Table
				{
					...
					ImGuiTableFlags tableFlags = ImGuiTableFlags_NoPadInnerX
						...
						/*| ImGuiTableFlagsRowBg *//*| ImGuiTableFlagsSortable*/;

					const int numColumns = 3;
					ImGui::BeginTable(UI::GenerateID(), numColumns, tableFlags, ImVec2(ImGui::GetContentRegionAvail()));
	...
	void SceneHierarchyPanel::DrawEntityNode(Entity entity, const std::string& searchFilter /*= {}*/)
	{
		...
		/*const bool isRowHovered = [&rowAreaMin, &rowAreaMax, &name, &entity]
		{
			bool hovered = ImGui::ItemHoverable(ImRect(rowAreaMin, rowAreaMax), (uint64_t)(uint32_t)entity);
		...
		const bool ismouse_x_over_arrow = (g.IO.MousePos.x >= arrow_hit_x1 && g.IO.MousePos.x < arrow_hit_x2);

		bool previousState = ImGui::TreeNodeBehaviorIsOpen(ImGui::GetID(strID.c_str()));

		if (ismouse_x_over_arrow && isRowClicked)
		...
		DrawComponent<MeshComponent>("Mesh", entity, [&](MeshComponent& mc)
			{
				...
				if (mesh && mesh->IsValid())
				{
					if (UI::BeginTreeNode("Materials"))
					{
						...
						/* // TODO(Yan): a reset all button?
						if (ImGui::Button("Reset") )
						{
							mc.Materials->Clear();
							if (mc.MeshHandle)
								mc.Materials->SetMaterialCount(mc.MeshHandle->GetMaterials()->GetMaterialCount());
						}*/
		...
		DrawComponent<AnimationComponent>("Animation", entity, [&](AnimationComponent& anim)
			{
				...
				if (AssetManager::IsAssetHandleValid(anim.AnimationController))
				{
					...
					if (UI::Property("State", stateIndex)) {
						animationController->SetStateIndex(std::clamp(stateIndex, 0, static_cast<int>(animationController->GetNumStates()) - 1));
					}

					UI::Property("Play Animation", anim.EnableAnimation);
					{
						UI::ScopedItemFlags flags(ImGuiItemFlags_Disabled, anim.EnableAnimation);
		...
		DrawComponent<ScriptComponent>("Script", entity, [=](ScriptComponent& sc) mutable
			{
				...
				if (UI::Property("Module Name", name)) // TODO: no live edit
				{
					// Shutdown old script
					if (!isError)
		...
		DrawComponent<AudioListenerComponent>("Audio Listener", entity, [&](AudioListenerComponent& alc)
			{
				...
				//? Have to manually clamp here because UI::Property doesn't take flags to pass in ImGuiSliderFlagsClampOnInput
