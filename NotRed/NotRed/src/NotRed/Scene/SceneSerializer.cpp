    ...
    static void SerializeEntity(YAML::Emitter& out, Entity entity)
    {
        ...
        if (entity.HasComponent<MeshComponent>())
        {
            ...
            if (materialTable->GetMaterialCount() > 0)
            {
                ...
                {
                    out << YAML::Key << i << YAML::Value << (materialTable->HasMaterial(i) ? materialTable->GetMaterial(i)->Handle : (AssetHandle)0);
                }
        ...
        if (entity.HasComponent<Audio::AudioComponent>())
        {
            ...
            {
                out << YAML::Key << "AssetID" << YAML::Value << soundConfig->FileAsset->Handle;
            }

            out << YAML::Key << "IsLooping" << YAML::Value << (bool)soundConfig->Looping;
            ...
            out << YAML::Key << "MasterReverbSend" << YAML::Value << soundConfig->MasterReverbSend;
            out << YAML::Key << "LPFilterValue" << YAML::Value << soundConfig->LPFilterValue;
            out << YAML::Key << "HPFilterValue" << YAML::Value << soundConfig->HPFilterValue;

            out << YAML::Key << "Spatialization";
            out << YAML::BeginMap; // Spatialization

            auto& spatialConfig = soundConfig->Spatialization;
            out << YAML::Key << "Enabled" << YAML::Value << soundConfig->SpatializationEnabled;
