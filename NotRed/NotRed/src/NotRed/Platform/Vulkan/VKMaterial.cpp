...
namespace NR
{
	...
	void VKMaterial::Invalidate()
	{
		...
		for (auto& descriptor : mResidentDescriptors)
		{
	...
	void VKMaterial::SetVulkanDescriptor(const std::string& name, const Ref<Texture2D>& texture)
	{
		...
		if (resource->GetRegister() < mTextures.size() && mTextures[resource->GetRegister()] && texture->GetHash() == mTextures[resource->GetRegister()]->GetHash())
		{
			return;
		}

		if (resource->GetRegister() >= mTextures.size())
		{
			mTextures.resize(resource->GetRegister() + 1);
		}
		mTextures[resource->GetRegister()] = texture;

		const VkWriteDescriptorSet* wds = mShader.As<VKShader>()->GetDescriptorSet(name);
		NR_CORE_ASSERT(wds);
		mResidentDescriptors.push_back(std::make_shared<PendingDescriptor>(PendingDescriptor{ PendingDescriptorType::Texture2D, *wds, {}, texture.As<Texture>(), nullptr }));
		mPendingDescriptors.push_back(mResidentDescriptors.back());
	}
	...
	void VKMaterial::SetVulkanDescriptor(const std::string& name, const Ref<TextureCube>& texture)
	{
		...
		const VkWriteDescriptorSet* wds = mShader.As<VKShader>()->GetDescriptorSet(name);
		NR_CORE_ASSERT(wds);
		mResidentDescriptors.push_back(std::make_shared<PendingDescriptor>(PendingDescriptor{ PendingDescriptorType::TextureCube, *wds, {}, texture.As<Texture>(), nullptr }));
		mPendingDescriptors.push_back(mResidentDescriptors.back());
	}
	...
	void VKMaterial::SetVulkanDescriptor(const std::string& name, const Ref<Image2D>& image)
	{
		...
		if (resource->GetRegister() < mImages.size() && mImages[resource->GetRegister()] && mImageHashes.at(resource->GetRegister()) == image->GetHash())
		{
			return;
		}
		...
		const VkWriteDescriptorSet* wds = mShader.As<VKShader>()->GetDescriptorSet(name);
		NR_CORE_ASSERT(wds);
		mResidentDescriptors.push_back(std::make_shared<PendingDescriptor>(PendingDescriptor{ PendingDescriptorType::Image2D, *wds, {}, nullptr, image.As<Image>() }));
		mPendingDescriptors.push_back(mResidentDescriptors.back());
	}
		...
		Renderer::Submit([instance]() mutable
			{
				...
				for (auto& descriptor : instance->mResidentDescriptors)
				{
