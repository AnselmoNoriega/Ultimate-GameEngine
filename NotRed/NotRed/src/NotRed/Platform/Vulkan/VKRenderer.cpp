...
namespace NR
{
	...
	void VKRenderer::DispatchComputeShader(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<PipelineCompute> pipeline, Ref<UniformBufferSet> uniformBufferSet, Ref<StorageBufferSet> storageBufferSet, Ref<Material> material, const glm::ivec3& workGroups, Buffer additionalUniforms)
	{
		...
		Renderer::Submit([renderCommandBuffer, vulkanPipeline, vulkanMaterial, uniformBufferSet, storageBufferSet, workGroups, pushConstantBuffer]() mutable
			{
		...
		Renderer::Submit([sceneRenderer, environment, shadow]() mutable
			{
				...
				const auto shader = Renderer::GetShaderLibrary()->Get("NotRedPBR_Static");
