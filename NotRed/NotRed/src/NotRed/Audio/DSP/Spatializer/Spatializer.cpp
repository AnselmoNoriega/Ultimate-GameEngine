...
namespace NR::Audio::DSP
{
    ...
    static void spatializer_node_process_pcm_frames(ma_node* pNode, const float** ppFramesIn, ma_uint32* pFrameCountIn, float** ppFramesOut, ma_uint32* pFrameCountOut)
    {
        ...
        const uint32_t numSourceInputs = vbap.ChannelGroups.size();
    ...
    {
        return mSources.find(sourceID) != mSources.end() && mSources.at(sourceID).bInitialized;
    }

    float Spatializer::GetCurrentDistanceAttenuation(uint32_t sourceID) const
    {
        NR_CORE_ASSERT(IsInitialized(sourceID))
            return mSources.at(sourceID).DistanceAttenuationFactor;
    }

    float Spatializer::GetCurrentConeAngleAttenuation(uint32_t sourceID) const
    {
        NR_CORE_ASSERT(IsInitialized(sourceID))
            return mSources.at(sourceID).AngleAttenuationFactor;
    }

    float Spatializer::GetCurrentDistance(uint32_t sourceID) const
    {
        NR_CORE_ASSERT(IsInitialized(sourceID));
        return mSources.at(sourceID).Distance;
    }
    ...
    void Spatializer::SetSpread(uint32_t sourceID, float newSpread)
    {
        ...
        {
            NR_CORE_ERROR("[Spatializer] - SetSpread({0}, {1}) - invalid sourseID.", sourceID, newSpread);
            return;
        }

        Source& source = mSources.at(sourceID);

        if (source.SpatializationConfig->bSpreadFromSourceSize)
        {
            NR_CORE_WARN("[Spatializer] Trying to update sound Spread value while using SpreadFromSourceSize.");
            return;
        }
    ...
    void Spatializer::SetFocus(uint32_t sourceID, float newFocus)
    {
        if (!IsInitialized(sourceID))
        {
            NR_CORE_ERROR("[Spatializer] - SetFocus({0}, {1}) - invalid sourseID.", sourceID, newFocus);
            return;
        }
    ...
    void Spatializer::UpdatePositionalData(Source& source, const ma_spatializer_listener* listener)
    {
        ...
        if (distance > 0)
        {
            ...
            if (listener != nullptr && listener->config.coneInnerAngleInRadians < 6.283185f)
            {
                ...
                //NR_CORE_TRACE("Angle Attenuation: {}", angleAttenuation);
            }
        ...
        //NR_CORE_TRACE("Gain attenuation: {}", gainAttenuation);
    ...
    {
        NR_CORE_ASSERT(mSources.find(sourceID) == mSources.end());

        //Source source;
        //source.sourceID = sourceID;
        const auto& [it, success] = mSources.emplace(sourceID, Source());
        ...
        ma_allocation_callbacks* allocationCallbacks = &mEngine->pResourceManager->config.allocationCallbacks;
        ...
        result = ma_node_init(&mEngine->nodeGraph, &nodeConfig, allocationCallbacks, &source.SpatializerNode);
    ...
    {
        auto sIt = mSources.find(sourceID);
        if (sIt == mSources.end())
        {
            NR_CORE_ERROR("[Sound Spatializer] Invalid sourceID {}.", sourceID);
            return false;
        }
    ...
    void Spatializer::UpdateSourcePosition(uint32_t sourceID, const Audio::Transform& transform, glm::vec3 velocity)
    {
        ...
        if (sIt == mSources.end())
        {
            NR_CORE_ERROR("[Spatializer] - UpdateSourcePosition() Invalid sourceID {}.", sourceID);
            return;
        }
        ...
        const auto& lp = mListenerTransform.Position;
        const auto& lr = mListenerTransform.Orientation;
        const auto& lup = mListenerTransform.Up;
        ...
        //NR_CORE_INFO("Relative Pos: X {0} | Y {1} | Z {2}", relativePos.x, relativePos.y, relativePos.z);

        //NR_CORE_TRACE("Angle degrees: {}", glm::degrees(azimuth));
        ...
        UpdatePositionalData(source, &mEngine->listeners[0]);
    ...
    void Spatializer::UpdateListener(const Audio::Transform& transform, glm::vec3 velocity)
    {
        ...
        std::for_each(std::execution::par_unseq, mSources.begin(), mSources.end(), [&](std::pair<const uint32_t, Source>& IDSourcePair)
            {
