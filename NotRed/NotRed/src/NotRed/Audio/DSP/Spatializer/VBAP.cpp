...
#pragma endregion
    ...
    bool VBAP::InitVBAP(VBAPData* vbap, const size_t numOfInputs, const size_t numOfOutputs, const ma_channel* sourceChannelMap, const ma_channel* outPutChannelMap)
    {
        NR_CORE_ASSERT(numOfInputs > 0 && numOfOutputs > 0);
        ...
        for (int i = 0; i < (int)numOfOutputs; ++i)
        {
            ...
            angles[i] = angle;
            //NR_CORE_TRACE("Angle degrees: {}", glm::degrees(angle));
        ...
        for (int i = 0; i < vbap->spPosSorted.size(); ++i)
        {
            ...
            //NR_CORE_TRACE("Sorted degrees: {}", glm::degrees(VectorAngle({ p.x, 0.0f, p.y })));
        ...
        for (uint32_t i = 0; i < numOfInputs; ++i)
        {
            ...
            /*const auto an = g_maChannelDirections[m_SurrChannelMap[i]];
              ...
              NR_CORE_TRACE("-----------------------------");
              NR_CORE_TRACE("Channel GRP ID {0}, angle: {1}, original angle {2}", id, channelAngle, originalAngle);*/

            for (int vi = 0; vi < vsPerChannel; ++vi)
            {
                sourceAngle += vsAngle;
                ...
                /*if (channelAngle < 0.0f)
                        NR_CORE_TRACE("Left Channel VS: channelAngle {0}, vsAngle {1}", channelAngle, sa);
                    else
                        NR_CORE_TRACE("Right Channel VS: channelAngle {0}, vsAngle {1}", channelAngle, sa);*/
    ...
    {
        NR_CORE_ASSERT(!vbap->VirtualSources.empty());
        ...
        for (auto& chg : vbap->ChannelGroups)
        {
            ...
            const int numVirtualSources = chg.VirtualSourceIDs.size();
            std::for_each(
                gainsLocal.begin(), gainsLocal.end(),
    ...
    void VBAP::ProcessVBAP(const VBAPData* vbap, const float azimuthRadians, ChannelGains& gains)
    {
        ...
        {
            //NR_CONSOLE_LOG_ERROR("Faild to find Active Arch.");
        }
    }
