...
namespace NR
{
	...
	public:
		...
		virtual Buffer<void> GetBuffer() const = 0;
		virtual Buffer<void>& GetBuffer() = 0;
	...
	{
	public:
		static Ref<Image2D> Create(ImageSpecification specification, Buffer<void> buffer);
	...
	namespace Utils {
		...
		inline uint32_t GetImageFormatBPP(ImageFormat format)
		{
			...
			default:
			{
				NR_CORE_ASSERT(false, "Can not handle this format!");
				return 0;
			}
			}
		}
