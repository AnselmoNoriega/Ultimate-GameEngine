...
namespace NR
{
    ...
    void SceneRenderer::Init()
    {
        ...
        // Particles Gen
        {
            int numParticles = 80128; // TODO: Change to particles global const
            int localSizeX = 256;
        ...
        // Particles
        {
            ...
            mParticlesPipeline = Pipeline::Create(pipelineSpecification);
        }
    ...
    void SceneRenderer::BeginScene(const SceneRendererCamera& camera)
    {
        ...
        if (mNeedsResize)
        {
            ...
            mParticlesPipeline->GetSpecification().RenderPass->GetSpecification().TargetFrameBuffer->Resize(mViewportWidth, mViewportHeight);
        ...
        UBStarParams& starParamsData = Particles::GetStarParams();
        UBEnvironmentParams& environmentParamsData = Particles::GetEnvironmentParams();
    ...
    void SceneRenderer::SubmitParticles(Ref<Particles> particles, const glm::mat4& transform)
    {
        NR_PROFILE_FUNC();

        ParticleData& particleKey = mParticlesDrawList.emplace_back();
        particleKey.ParticlesRef = particles;
        particleKey.Material = particles->GetMaterial();

        auto& transformStorage = mParticlesTransformMap[particleKey].Transforms.emplace_back();
    ...
    void SceneRenderer::GeometryPass()
    {
        ...
        // Render particles
        for (auto& particlesData : mParticlesDrawList)
        {
            const auto& transformData = mParticlesTransformMap.at(particlesData);
            Renderer::RenderParticles(mCommandBuffer, mParticlesPipeline, mUniformBufferSet, mStorageBufferSet, particlesData.ParticlesRef, particlesData.Material, mSubmeshTransformBuffer, transformData.TransformOffset);
        }
