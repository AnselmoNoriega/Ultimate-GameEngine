...
namespace ImGui
{
    bool TreeNodeWithIcon(NR::Ref<NR::Texture2D> icon, ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, ImColor iconTint/* = IM_COL32_WHITE*/)
    {
        ...
        if (!is_leaf)
        {
            ...
            if (g.NavId == id && g.NavAnyRequest && g.NavMoveDir == ImGuiDir_Left && is_open)
            {
                toggled = true;
                NavMoveRequestCancel();
            }

            if (g.NavId == id && g.NavAnyRequest && g.NavMoveDir == ImGuiDir_Right && !is_open) // If there's something upcoming on the line we may want to give it the priority?
            {
                toggled = true;
                NavMoveRequestCancel();
            }
