...
namespace NR::Script
{
    ...
    static void AddCollidersToArray(MonoArray* array, const std::array<OverlapHit, OVERLAP_MAX_COLLIDERS>& hits, uint32_t count, uint32_t arrayLength)
    {
        ...
        for (uint32_t i = 0; i < count; i++)
        {
            ...
            if (entity.HasComponent<MeshColliderComponent>() && arrayIndex < arrayLength)
            {
                ...
                MonoObject* obj = ScriptEngine::Construct("NR.MeshHandleCollider:.ctor(ulong,bool,intptr)", true, data);
                mono_array_set(array, MonoObject*, arrayIndex++, obj);
            }
        }
    }
    ...
    {
        return NR_Entity_InstantiateWithTransform(entityID, prefabID, inPosition, nullptr, nullptr);
    }
    ...
    uint64_t NR_Entity_InstantiateWithTransform(uint64_t entityID, uint64_t prefabID, glm::vec3* inPosition, glm::vec3* inRotation, glm::vec3* inScale)
    {
        ...
        Ref<Prefab> prefab = AssetManager::GetAsset<Prefab>(prefabID);
        Entity result = scene->Instantiate(prefab, inPosition, inRotation, inScale);
        return result.GetID();
    }
    ...
    void* NR_MeshComponent_GetMesh(uint64_t entityID, bool* outIsStatic)
    {
        ...
        if (entity.HasComponent<StaticMeshComponent>())
        {
            ...
            *outIsStatic = true;
            return new Ref<StaticMesh>(mesh);
        }
        return nullptr;
    }
    ...
    void NR_MeshComponent_SetMesh(uint64_t entityID, void* in, bool inIsStatic)
    {
        ...
        {
            auto& meshComponent = entity.GetComponent<StaticMeshComponent>();
            Ref<StaticMesh>* inMesh = (Ref<StaticMesh>*)in;
            meshComponent.StaticMesh = inMesh ? (*inMesh)->Handle : AssetHandle(0);
        }
        else
        {
            auto& meshComponent = entity.GetComponent<MeshComponent>();
            Ref<Mesh>* inMesh = (Ref<Mesh>*)in;
    ...
    bool NR_MeshComponent_GetIsRigged(uint64_t entityID)
    {
        auto entity = GetEntity(entityID);
        auto& meshComponent = entity.GetComponent<MeshComponent>();
        return AssetManager::IsAssetHandleValid(meshComponent.MeshHandle) && AssetManager::GetAsset<Mesh>(meshComponent.MeshHandle)->IsRigged();
    }
    ...
    bool NR_AnimationComponent_GetIsAnimationPlaying(uint64_t entityID)
    {
        ...
        return AssetManager::IsAssetHandleValid(animationComponent.AnimationController) && AssetManager::GetAsset<AnimationController>(animationComponent.AnimationController)->IsAnimationPlaying();
    }
    ...
    void NR_AnimationComponent_SetIsAnimationPlaying(uint64_t entityID, bool value)
    {
        ...
        {
            AssetManager::GetAsset<AnimationController>(animationComponent.AnimationController)->SetAnimationPlaying(value);
        }
    }
    ...
    uint32_t NR_AnimationComponent_GetStateIndex(uint64_t entityID)
    {
        auto entity = GetEntity(entityID);
        auto& animationComponent = entity.GetComponent<AnimationComponent>();
        return AssetManager::IsAssetHandleValid(animationComponent.AnimationController) ? AssetManager::GetAsset<AnimationController>(animationComponent.AnimationController)->GetStateIndex() : 0;
    }
    ...
    void NR_AnimationComponent_SetStateIndex(uint64_t entityID, uint32_t value)
    {
        ...
        if (AssetManager::IsAssetHandleValid(animationComponent.AnimationController))
        {
            AssetManager::GetAsset<AnimationController>(animationComponent.AnimationController)->SetStateIndex(value);
        }
    }
    ...
    void NR_RigidBody2DComponent_SetPosition(uint64_t entityID, glm::vec2* inPosition)
    {
        ...
        auto& component = entity.GetComponent<RigidBody2DComponent>();
        b2Body* body = (b2Body*)component.RuntimeBody;
        body->SetTransform(b2Vec2(inPosition->x, inPosition->y), body->GetAngle());
    }
    ...
    void NR_RigidBodyComponent_SetBodyType(uint64_t entityID, RigidBodyComponent::Type type)
    {
        ...
        //auto actor = scene->GetPhysicsWorld()->GetActor(entity);
    ...
    void NR_RigidBodyComponent_SetVelocity(uint64_t entityID, glm::vec3* inVelocity)
    {
        ...
        ScriptEngine::GetCurrentSceneContext()->GetPhysicsScene()->GetActor(entity)->SetVelocity(*inVelocity);
    }
    ...
    void NR_FixedJointComponent_SetIsBreakable(uint64_t entityID, bool isBreakable)
    {
        ...
        Ref<NR::JointBase> joint = ScriptEngine::GetCurrentSceneContext()->GetPhysicsScene()->GetJoint(entity);
        if (isBreakable)
        {
    ...
    void* NR_MeshColliderComponent_GetColliderMesh(uint64_t entityID, bool* outIsStatic)
    {
        ...
        {
            auto mesh = AssetManager::GetAsset<StaticMesh>(component.CollisionMesh);
            *outIsStatic = true;
            return new Ref<StaticMesh>(new StaticMesh(mesh->GetMeshSource()));
        }
        else
        {
    ...
    void* NR_Mesh_Constructor(MonoString* filepath, bool* outIsStatic)
    {
        ...
        if (AssetManager::IsAssetHandleValid(handle))
        {
            ...
            {
                *outIsStatic = true;
                return new Ref<StaticMesh>(new StaticMesh(AssetManager::GetAsset<MeshSource>(handle)));
            }
        ...
        *outIsStatic = true;
        return new Ref<StaticMesh>(new StaticMesh(Ref<MeshSource>::Create(mono_string_to_utf8(filepath))));
    }
    ...
    void NR_Mesh_Destructor(void* _this, bool isStatic)
    {
        if (isStatic)
        {
            Ref<StaticMesh>* instance = (Ref<StaticMesh>*)_this;
            delete instance;
        }
        else
        {
    ...
    Ref<MaterialAsset>* NR_Mesh_GetMaterial(void* inMesh, bool isStatic)
    {
        if (isStatic)
        {
            ...
            if (!inMesh || !mesh->IsValid())
                return nullptr;

            const auto& materialTable = mesh->GetMaterials();
            NR_CORE_ASSERT(materialTable->GetMaterialCount() > 0);
            return new Ref<MaterialAsset>(materialTable->GetMaterial(0));
        }
        else
        {
            Ref<StaticMesh>& mesh = *(Ref<StaticMesh>*)inMesh;
            if (!inMesh || !mesh->IsValid())
                return nullptr;
    ...
    void* NR_MeshFactory_CreatePlane(float width, float height)
    {
        ...
        return new Ref<StaticMesh>(new StaticMesh(Ref<MeshSource>::Create("assets/models/Plane1m.obj")));
    }
