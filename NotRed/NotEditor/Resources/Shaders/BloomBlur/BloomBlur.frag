...
void main()
{
    ...
    if (uHorizontal)
    {
	    for(int i = 1; i < 5; ++i)
        {
            result += texture(uTexture, vTexCoord + vec2(tex_offset.x * i, 0.0)).rgb * weight[i];
            result += texture(uTexture, vTexCoord - vec2(tex_offset.x * i, 0.0)).rgb * weight[i];
        }
    }
    else
    {
        for(int i = 1; i < 5; ++i)
        {
            result += texture(uTexture, vTexCoord + vec2(0.0, tex_offset.y * i)).rgb * weight[i];
            result += texture(uTexture, vTexCoord - vec2(0.0, tex_offset.y * i)).rgb * weight[i];
        }
    }
    ...
    vec2 Radius = Size/textureSize(uTexture, 0);

    vec3 result = texture(uTexture, vTexCoord).rgb;
    vec2 uv = vTexCoord;
    // Blur calculations
    for( float d=0.0; d<Pi; d+=Pi/Directions)
    {
        ...
        {
			result += texture( uTexture, uv+vec2(cos(d),sin(d))*Radius*i).rgb;
        }
    }
