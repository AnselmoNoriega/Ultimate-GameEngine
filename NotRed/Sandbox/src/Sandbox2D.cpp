...
void Sandbox2D::Attach()
{
    ...
    NR::FramebufferStruct fbSpecs;
    fbSpecs.Width = 1280;
    fbSpecs.Height = 720;
    mFramebuffer = NR::Framebuffer::Create(fbSpecs);
}
...
void Sandbox2D::ImGuiRender()
{
	...
	if (dockingEnabled)
	{
		...
		if (ImGui::BeginMenuBar())
		{
			if (ImGui::BeginMenu("File"))
			{
				...
				if (ImGui::MenuItem("Exit")) NR::Application::Get().Close();
				ImGui::EndMenu();
			}
			ImGui::EndMenuBar();
		}
	...
	else
	{
		...
		uint32_t textureID = mTexture->GetRendererID();
